...
exports.init = function (cli) {
  ...
  */
                ...
                if (!decision) {
                  ...
                  mobile.recover(options, function (error) {
                    if (error) {
                      ...
                      options.servicename = destinationservicename;
  ...
  mobile.command('migrate [sourceservicename] [destinationservicename]')
        ...
        .description($('Migrates a mobile service using the capacity reserved by a healthy mobile service in a different region. This command is for disaster recover scenarios and it may result in up to 30 minutes of downtime as the migration completes.'))
        .option('-q, --quiet', $('do not prompt for confirmation of recovery'))
        .option('-s, --subscription <id>', $('the subscription id'))
        .execute(function (sourceservicename, destinationservicename, options, callback) {
          if (sourceservicename) {
            ...
            promptString($('Name of the source mobile service to recover: '), ensuredSourceServiceName);
          }
          ...
          function ensuredSourceServiceName(sourceservice) {
            ...
            function ensuredDestinationServiceName(destinationservice) {
              ...
              if (options.quiet) {
                ...
                cli.confirm(util.format($('Warning: this action will use the capacity from the mobile service \'%s\' and delete it and the host name for \'%s\' may not resolve for up to 30 minutes. Do you want to recover the mobile service \'%s\'? [y/n]: '),
  ...
  mobile.waitForMigration = function (options, callback) {
    function checkMigrationStatus() {
      mobile.getMigrationState(options, function (error, body) {
        if (error) {
          return callback(new Error($('Unable to determine the status of the async operation. Please check the status on the management portal.')));
        }
        var status = body;
